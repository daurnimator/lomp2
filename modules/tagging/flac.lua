...
if s == "fLaC" then 
	...
	repeat
		...
		local lastmetadatablock = vstruct.implode { unpack ( t [ 1 ] , 1 , 1 ) }
		local blocktype = vstruct.implode { unpack ( t [ 1 ] , 2 , 8 ) }
		...
		if blocktype == 0 then -- Stream info
			...
			sampleinfo.samplerate = vstruct.implode { unpack ( t [ 5 ] , 1 , 20 ) }
			sampleinfo.channels = vstruct.implode { unpack ( t [ 5 ] , 21 , 23 ) } + 1
			sampleinfo.bitspersample = vstruct.implode { unpack ( t [ 5 ] , 24 , 28 ) }
			sampleinfo.totalsamples = vstruct.implode { unpack ( t [ 5 ] , 29 , 64 ) }
		...
		elseif blocktype == 2 then -- Application
			t = vstruct.unpack ( ">u4s" .. ( blocklength - 4 ) , fd )
			appID = t [ 1 ]
			appdata = t [ 2 ]
		...
		elseif blocktype == 4 then -- Vorbis_Comment http://www.xiph.org/vorbis/doc/v-comment.html
			...
			comment = { }
			tags = { }
			for i = 1 , ( user_comment_list_length + 1 ) do
				t = vstruct.unpack ( "<u4" , fd )
				local length = t [ 1 ]
