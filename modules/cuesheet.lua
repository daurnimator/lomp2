...
do
	...
	local indexnumber = C ( digit * digit^-1 / tonumber )
	local INDEX = C "INDEX" * space * indexnumber * space * timestamp
	
	local isrccode = C ( alphanumeric * alphanumeric * alphanumeric * alphanumeric * alphanumeric * digit * digit * digit * digit * digit * digit * digit / strupper )
	local ISRC = C "ISRC" * space * isrccode
	...
	local datatype = C ( P "AUDIO" + "CDG" + "MODE1/2048" + "MODE1/2352" + "MODE2/2336" + "MODE2/2352" + "CDI/2336" + "CDI/2352" ) + function ( _ , i ) error ( "Invalid track datatype in cuesheet" ) end
	local TRACK = C "TRACK" * space * indexnumber * space * datatype
	
	lineparser = space^0 * ( CATALOG + CDTEXTFILE + FILE + FLAGS + INDEX + ISRC + PERFORMER + POSTGAP + PREGAP + REM + SONGWRITER + TITLE + TRACK )^-1 * space^0 * eos + function ( _ , i ) error ( "Invalid cuesheet" ) end
	
	commentparser = C ( upper^1 ) * space^1 * str
end
...
local d = {
	...
	FILE = function ( data , path , typ )
		...
		files [ #files + 1 ] = { path = path , type = typ , tracks = { } }
	end ;
	FLAGS = function ( data , ... )
		local files = data.files
		if #files > 0 then
			...
			if #tracks > 0 then
				local flags = tracks [ track ].flags
	...
	INDEX = function ( data , index , frames )
		...
		if #files > 0 then
			local tracks = files [ #files ].tracks
			if #tracks > 0 then
				local indexes = tracks [ track ].indexes
	...
	PERFORMER = function ( data , performer )
		...
		if #files > 0 then
			local tracks = files [ #files ].tracks
			if #tracks > 0 then
				tracks [ track ].performer = performer
				return
			end
		end
	...
	SONGWRITER = function ( data , songwriter )
		...
		if #files > 0 then
			local tracks = files [ #files ].tracks
			if #tracks > 0 then
				tracks [ track ].songwriter = songwriter
				return
			end
		end
	...
	TRACK = function ( data , track , datatype )
		...
		if #files > 0 then
			local tracks = files [ #files ].tracks
			tracks [ #tracks + 1 ] = { type = datatype , track = track , postgap = 0 , pregap = 0 , flags = { } , indexes = { } ,
